...
async function loadEstimateTemplate()
{
    try{
        ...
        data.templates
        .filter(template => template.name === "Construction Estimate")
        .forEach(template => {
            const tile = document.createElement("div");
            tile.className = "template-tile3";
            tile.onclick = () => renderEstimateForm(template);
...
function renderEstimateForm(template){
