...
async function loadProdCommunTemps(){
    const response = await fetch("/demo/src/main/resources/static/data/marketing.json");
    const data = await response.json();
    const gallery = document.getElementById("template-gallery");

    data.templates
    .filter(template => template.name == "Product Launch Communication Plan")
    .forEach(template => {
        const tile = document.createElement("div");
        tile.className = "template-tile3";

        tile.onclick = () => renderCommForm(template);
...
function renderCommForm(template) {
        ...
        if(field.type == 'dynamic_table'){
            ...
            if(columns){
                ...
                field.columns.forEach(() => {
                    ...
                    cellInput.type = "text";
                    cellInput.className = "table-input1";
